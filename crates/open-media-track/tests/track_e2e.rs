...
use open_media_core::ports::{Enricher, Tracker};
use open_media_core::tracking::ListStatus;
use open_media_track::{AniListTracker, AniSkipEnricher, MalTracker};
use serde_json::json;
...
async fn mal_tracker_maps_401_to_auth_error() {
    ...
    let err = tracker
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, open_media_core::error::CoreError::Auth(_)));
}
